...
class CGDLTextureManager {
  ...
  static loadUtils(): void {
    ...
    CGDLTextureManager.cgdl_canvas = document.getElementById(
      "cgdl-canvas"
  ...
  static cgdlInit(bid: string): void {
    if (!CGDLTextureManager.cgdl_first_init) {
      ...
      document
        ...
        .addEventListener("load", CGDLTextureManager.__cgdlSpanLoader);
      ...
      CGDLTextureManager.cgdl_span_clock = setInterval(
        () => {
          ...
          if (
            CGDLTextureManager.cgdl_load_id >=
            CGDLTextureManager.cgdl_src_list.length
          ) {
            flag = false;
            if (
              !CGDLTextureManager.cgdl_img_loading &&
              CGDLTextureManager.cgdl_span_clock_img_loading
            ) {
        ...
        () => {
          if (!CGDLTextureManager.cgdl_img_main_loop_prev_running) {
            ...
            ctx.clearRect(
              0,
              0,
              CGDLTextureManager.cgdl_canvas.width,
              CGDLTextureManager.cgdl_canvas.height
            );
            for (let cmd of CGDLTextureManager.cgdl_img_commands) {
              if (!CGDLTextureManager.cgdl_img_loaded[cmd.img_id]) continue; // undecided: break?
  ...
  static cgdlDrawImg(
    ...
    CGDLTextureManager.cgdl_img_commands.push({
      img_id: img_id,
      x: x,
      y: y,
      width: w,
  ...
  static cgdlModifyImg(
    ...
    if (x_app) x = CGDLTextureManager.cgdl_img_commands[cmd_id].x + x;
    if (y_app) y = CGDLTextureManager.cgdl_img_commands[cmd_id].y + y;
    if (w_app) w = CGDLTextureManager.cgdl_img_commands[cmd_id].width + w;
    if (h_app) h = CGDLTextureManager.cgdl_img_commands[cmd_id].width + h;
    CGDLTextureManager.cgdl_img_commands[cmd_id] = {
      img_id: img_id,
      x: x,
      y: y,
      width: w,
