// ontributors: Twilight
// D Game Design Library PROTOTYPE
"use strict"
...
class CGDLTextureManager {
  static UtilsLoader() {
    CGDLTextureManager.cgdl_src_list = new Array();                             // he list of the directories of the image resources
    CGDLTextureManager.cgdl_img_loaded = new Array();                           // hether the image resources have been loaded
    CGDLTextureManager.cgdl_span_load = false;                                  // hether the resource span has been loaded
    CGDLTextureManager.cgdl_span_clock = null;                                  // he clock checking the loading state of the resource span, see function @_cgdlSpanLoader()
    CGDLTextureManager.cgdl_first_init = false;                                 // hether the module has already been initialized, see function @cgdlInit(bid)
    CGDLTextureManager.cgdl_img_loading = true;                                 // hether the images have all been uploaded as resources, see function @cgdlEndAppending()
    CGDLTextureManager.cgdl_img_all_loaded = false;                             // hether the images have all finished loading
    CGDLTextureManager.cgdl_load_id = 0;                                        // he current loading image
    CGDLTextureManager.cgdl_span_clock_img_loading = true;                      // hether the images have all started loading, see function @cgdlInit(bid)
    CGDLTextureManager.cgdl_img_main_loop = null;                               // he clock controlling the canvas and all the commands, see function @cgdlInit(bid)
    CGDLTextureManager.cgdl_img_main_loop_prev_running = false;                 // hether the main-loop clock is still in running when one or several intervals have passed, it may be deprocated.
    CGDLTextureManager.cgdl_img_commands = new Array();                         // ach element is an array, [img_id: int, x: int, y: int, width: int, height: int]
    CGDLTextureManager.cgdl_canvas = document.getElementById("cgdl-canvas");  // he dom of the canvas
  }

  /**
    * function __cgdlRunCmd : draw the image by context @ctx as the command @cmd required to. Please do not call this function manually or something unexpected may happen.
  ...
  static __cgdlRunCmd(cmd, ctx) {
    ...
    let img_w = cmd[3];
    let img_h = cmd[4];
    let img = document.getElementById("cgdl-resource-img-" + img_id);
  ...
  static cgdlInit(bid) {
    if (!CGDLTextureManager.cgdl_first_init) {
      ...
      // fter the span tag is loaded, the images will start loading
      CGDLTextureManager.cgdl_span_clock = setInterval(() => {
        let flag = true;
        let end_flag = true;
        if (!CGDLTextureManager.cgdl_span_clock_img_loading) {
          flag = false;
          for (let i in CGDLTextureManager.cgdl_img_loaded) {
      ...
      CGDLTextureManager.cgdl_img_main_loop = setInterval(() => {
        if (!CGDLTextureManager.cgdl_img_main_loop_prev_running) {
          ...
          for (let i in CGDLTextureManager.cgdl_img_commands) {
            let cmd = CGDLTextureManager.cgdl_img_commands[i];
            if (CGDLTextureManager.cgdl_img_loaded[cmd[0]] == false)
  ...
  static _cgdlAppendSrc(id) {
    ...
    let img = '<img id="cgdl-resource-img-' + id + '" src="' + src + '" height="0" width="0" onload="CGDLTextureManager._cgdlLoader(' + id + ')"/>'
  ...
  static cgdlModifyImg(cmd_id, img_id, x, y, w, h) {
    if (img_id == null)
      img_id = CGDLTextureManager.cgdl_img_commands[cmd_id][0];
    if (x == null)
      x = CGDLTextureManager.cgdl_img_commands[cmd_id][1];
    if (y == null)
      y = CGDLTextureManager.cgdl_img_commands[cmd_id][2];
    if (w == null)
      w = CGDLTextureManager.cgdl_img_commands[cmd_id][3];
    if (h == null)
      h = CGDLTextureManager.cgdl_img_commands[cmd_id][4];
  ...
  static cgdlModifyImg(cmd_id, img_id, x, y, w, h, x_app, y_app, w_app, h_app) {
    ...
    if (x == null)
      x = CGDLTextureManager.cgdl_img_commands[cmd_id][1];
    if (x_app == true)
      x = CGDLTextureManager.cgdl_img_commands[cmd_id][1] + x;
    if (y == null)
      y = CGDLTextureManager.cgdl_img_commands[cmd_id][2];
    if (y_app == true)
      y = CGDLTextureManager.cgdl_img_commands[cmd_id][2] + y;
    if (w == null)
      w = CGDLTextureManager.cgdl_img_commands[cmd_id][3];
    if (w_app == true)
      w = CGDLTextureManager.cgdl_img_commands[cmd_id][3] + w;
    if (h == null)
      h = CGDLTextureManager.cgdl_img_commands[cmd_id][4];
    if (h_app == true)
      h = CGDLTextureManager.cgdl_img_commands[cmd_id][4] + h;
    CGDLTextureManager.cgdl_img_commands[cmd_id] = [img_id,x,y,w,h];
  }
  ...
  // oad three images, when running the programme, please change the directory of the images
  let src1 = "C:/Users/admin/Desktop/timg.jpg";
  let src2 = "C:/Users/admin/Desktop/Twilight.png";
  let src3 = "C:/Users/admin/Desktop/Twilight2.png";

  // oad the images as resources
  let id1 = ctm.cgdlAppendSrc(src1);
  let id2 = ctm.cgdlAppendSrc(src2);
  let id3 = ctm.cgdlAppendSrc(src3);

  // nd of importing image resources (Not neccessary)
  ctm.cgdlEndAppending();

  // raw all the images on the canvas
  ...
  // ake the images move and scale
  let demo_movement = setInterval(function() {
